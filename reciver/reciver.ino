...
void loop() {
  if (ET.receiveData()) {
    if (mydata.receiver_id == 0 || mydata.receiver_id == this_device_id) {     // 0 - broadcast, or this device id
      ...
      mydata.crypt = k.decrypt(mydata.crypt);     // decrypt data
      unsigned int random_number = floor((mydata.crypt - mydata.receiver_id) / 100000);     // calculate a random number
      if (random_number == mydata.random_number) {      // compare with an unencrypted number
        unsigned int obtained_id = mydata.crypt - random_number * 100000;     // calculate obtained id of transmitter
        for (byte i = 0; i < count_of_transmitters; i++) {     // verify transmitter id
          if (transmitter_id[i] == obtained_id) {
            if (packet_id[i] == mydata.packet_id) {
              packet_id[i]++;
